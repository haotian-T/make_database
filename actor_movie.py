...
try:
    cur = db.cursor()
    cur.execute('DROP TABLE IF EXISTS actor_movie')
    create = """
    ...
    """
    cur.execute(create)
    print('actor_movie 表格创建成功')
except pymysql.Error as err:
    print(err)
    print("actor_movie 表格创建失败")
try:
    ...
    for i in range(len(reader)):
        if i == 0:
            ...
            for actor in line['actors']:
                datas.append([line['id'], actor])

    print('共' + str(len(datas)))
    time.sleep(1)
    sql = "INSERT INTO actor_movie(id,actor) VALUES (%s,%s)"
    ...
    # 提交到数据库执行
    db.commit()
    print("actor_movie 插入数据成功")
except pymysql.Error as err:
    print(err)
    print("actor_movie 插入数据失败")
    db.rollback()
try:
