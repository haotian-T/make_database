...
try:
    cur = db.cursor()
    cur.execute('DROP TABLE IF EXISTS region_movie')
    create = """
        CREATE TABLE `region_movie`(
        `id` int NOT NULL,
        `region` VARCHAR (255) DEFAULT NULL
        )
    """
    cur.execute(create)
    print('region_movie 表格创建成功')
except pymysql.Error as err:
    print(err)
    print("region_movie 表格创建失败")
try:
    ...
    for i in range(len(reader)):
        if i == 0:
            ...
            for region in line['regions']:
                datas.append([line['id'], region])

    print('共' + str(len(datas)))
    time.sleep(1)
    sql = "INSERT INTO region_movie(id,region) VALUES (%s,%s)"
    ...
    # 提交到数据库执行
    db.commit()
    print("region_movie 插入数据成功")
except pymysql.Error as err:
    print(err)
    print("region_movie 插入数据失败")
    db.rollback()
try:
